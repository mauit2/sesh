...
for (const [iso, list] of byCountry) {
  ...
  for (const c of cities) {
    ...
    for (const lang of langs) {
      ...
      if (lang === "en") {  // one globe marker per city (English flagship URL)
        intlGlobe.push({
          city: c.city, country: countryName,
          lat: Number(c.lat.toFixed(3)), lon: Number(c.lon.toFixed(3)),
          med: Math.round(c.med * 100) / 100, cur: C.cur, bars: c.cheap.length,
          url: urlFor("en", "cheap", c), home: false,
        });
      }
      { // cheapest page
...
// ---------------------------------------------------------------- globe
// The homepage world globe: every blog city as a dot, plus a daily-cached FX
// table so "in your currency" works without a runtime API or CSP change.
{
  let seGlobe = [];
  try { seGlobe = JSON.parse(readFileSync(join(HERE, ".se-summary.json"), "utf8")).seGlobe || []; }
  catch { /* gen-blog.mjs not run — Swedish dots absent this pass */ }
  const markers = [...seGlobe, ...intlGlobe]
    // only keep a dot whose blog page actually exists on disk
    .filter((m) => existsSync(join(ROOT, m.url.replace(`${SITE}/`, ""), "index.html")));

  // Daily-cached FX (USD base). Fetched at build; falls back to last-known.
  const FX_FALLBACK = { USD:1, EUR:0.92, GBP:0.79, SEK:10.5, CHF:0.88, CAD:1.36, AUD:1.52, NZD:1.65, THB:35, SGD:1.35, HKD:7.8, BRL:5.4 };
  let fx = FX_FALLBACK;
  try {
    const r = await fetch("https://open.er-api.com/v6/latest/USD");
    const j = await r.json();
    if (j && j.result === "success" && j.rates) fx = j.rates;
  } catch { /* offline build — keep fallback */ }

  writeFileSync(join(ROOT, "globe-cities.js"),
    `/* Generated by gen-blog-intl.mjs — do not edit by hand. */\n` +
    `window.SEJDEL_GLOBE_CITIES=${JSON.stringify(markers)};\n` +
    `window.SEJDEL_FX=${JSON.stringify(fx)};\n` +
    `window.SEJDEL_FX_BASE="USD";\n` +
    `window.SEJDEL_FX_UPDATED=${JSON.stringify(updated)};\n`);
  console.log(`globe-cities.js: ${markers.length} dots, FX ${Object.keys(fx).length} currencies`);
}
